...

{
	...
	if ((*A).l < 1 || (*A).c < 1) {
		...
		dmatrit_t * subbed;
		for (i = 1; i <= (*A).c; i++) {
			subbed=sub_matrix(A,1,i);
			det += pow(-1.0, i + 1.0)*(*A).m[1][i] * determinant(subbed);
		}
		delete_dmatrix(subbed);
	}
	return det;
}
...

{
	...
	for (i = 1; i <= (*A).l; i++) {
		for (j = 1; j <= (*A).c; j++) {
			subbed=sub_matrix(A,i,j);
			(*B).m[i][j] = pow(-1.0, i + j)*determinant(subbed);
		}
		delete_dmatrix(subbed);
	}
	return B;
...
{
	dmatrix_t *cf=cofactor(A);
	dmatrix_t *cfT =dmat_transpose(cf);
	dmatrix_t *detA= determinant(A);
	dmatrix_t *Ans =dmat_scalar_mult(cfT,1.0/detA);
